...
class CropCalendarView(APIView):
    ...
    def get(self, request):
        ...
        seasons = {
            'kharif': {'months': ['June', 'July', 'August', 'September', 'October'], 'label': 'Kharif (Monsoon)'},
            'rabi': {'months': ['November', 'December', 'January', 'February', 'March'], 'label': 'Rabi (Winter)'},
            'zaid': {'months': ['April', 'May', 'June'], 'label': 'Zaid (Summer)'},
            'all': {'months': ['All Year'], 'label': 'All Seasons'}
        }

        calendar_data = []
        for key, info in seasons.items():
            crops = Crop.objects.filter(season=key).order_by('order', 'name')
            if crops.exists():
        ...
        # Also handle 'all' season crops by adding them to relevant seasons if needed,
