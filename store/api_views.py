...
class RecommendationsView(APIView):
    ...
    def get(self, _request, product_id):
        ...
        category_products = list(Product.objects.select_related('category', 'vendor').filter(
            category=product.category,
            is_active=True
        ).exclude(id=product.id)[:4])

        # Create exclusion list
        excluded_ids = {p.id for p in category_products}
        excluded_ids.add(product.id)

        # Get products from same vendor
        vendor_products = list(Product.objects.select_related('category', 'vendor').filter(
            vendor=product.vendor,
            is_active=True
        ).exclude(id__in=excluded_ids)[:2])

        excluded_ids.update(p.id for p in vendor_products)

        # Get products in similar price range (±20%)
        price_min = float(product.price) * 0.8
        price_max = float(product.price) * 1.2
        price_products = list(Product.objects.select_related('category', 'vendor').filter(
