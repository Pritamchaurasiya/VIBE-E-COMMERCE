...
class RecommendationsView(APIView):
    ...
    def get(self, _request, product_id):
        ...
        # Get IDs to exclude
        exclude_ids = {p.id for p in category_products}
        ...
        vendor_products = list(Product.objects.filter(
            ...
            is_active=True
        ).exclude(id__in=exclude_ids).select_related('category', 'vendor')[:2])

        # Add new IDs to exclude
        exclude_ids.update(p.id for p in vendor_products)
