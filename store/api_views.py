...
class ProductListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # Optimization: Prefetch images and annotate review stats to prevent N+1 queries
    ...
    def list(self, request, *args, **kwargs):
        ...
        # Optimization: Fetch wishlist IDs for the current user in one go
        ...
        # Helper to get serializer with optimization context
        def get_optimized_serializer(data, many=True):
            context = self.get_serializer_context()
            context['wishlist_product_ids'] = wishlist_product_ids
            return self.serializer_class(data, many=many, context=context)
        ...
        if limit:
            ...
            serializer = get_optimized_serializer(queryset, many=True)
            return Response(serializer.data)
        else:
            # Use pagination
            page = self.paginate_queryset(queryset)
            if page is not None:
                serializer = get_optimized_serializer(page, many=True)
