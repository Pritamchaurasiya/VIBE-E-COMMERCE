...
class VendorAnalyticsAPIView(APIView):
    ...
    def get(self, request):
        ...
        # Calculate metrics using DB aggregation to avoid N+1 and memory issues
        revenue_data = order_items.aggregate(
            total=Sum(models.F('price') * models.F('quantity'), output_field=models.DecimalField()),
            monthly=Sum(
                models.Case(
        ...
        monthly_revenue = revenue_data.get('monthly') or 0
        weekly_revenue = revenue_data.get('weekly') or 0
