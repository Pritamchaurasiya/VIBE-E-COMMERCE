...
class CouponService:
    ...
    def apply_coupon(self, code: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Use atomic update with F expression to prevent race conditions
            # Also ensure usage limit is respected atomically
            ...
            if rows_updated == 1:
                logger.info("Coupon %s applied", code)
                return True, "Coupon applied successfully"
            else:
                # Check if it was limit reached or something else (though unlikely given get succeeded)
                if coupon.used_count >= coupon.max_uses:
