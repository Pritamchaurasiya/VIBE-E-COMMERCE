...
class CouponService:
    ...
    def apply_coupon(self, code: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # This prevents race conditions where multiple requests might
            # increment beyond max_uses
            ...
            if rows_updated == 0:
                 # Check why it failed (max uses or doesn't exist anymore)
                 coupon.refresh_from_db()
                 if coupon.used_count >= coupon.max_uses:
                     logger.warning("Coupon %s reached usage limit: %d/%d",
                                   code, coupon.used_count, coupon.max_uses)
