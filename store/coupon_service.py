...
class CouponService:
    ...
    def apply_coupon(self, code: str) -> Tuple[bool, str]:
        ...
        try:
            # Atomic update to prevent race conditions and enforce usage limits
            # pylint: disable=no-member
            ...
            if updated_count == 0:
                # Check if it was because it doesn't exist or because it's exhausted
                if not Coupon.objects.filter(code__iexact=code.strip()).exists():
                    return False, "Coupon not found"
                return False, "Coupon usage limit reached"

            logger.info("Coupon %s applied successfully (atomic increment)", code)
