...
# Newly added models to support Tracking Service

class SystemAccessTracker(models.Model):
    """
    Model for tracking all system access attempts (login, API, admin, etc.)
    """
    ...
    def __str__(self):
        return f"{self.access_type} - {self.resource_accessed} ({self.access_timestamp})"


class SessionTracker(models.Model):
    """
    Model for tracking user sessions independently of Django's session backend.
    """
...
class TrackingDataRetention(models.Model):
    ...
    retention_unit = models.CharField(max_length=10) # days, weeks, months, years
